...
export async function GET(request: NextRequest) {
  ...
  if (searchParams.get("minProductivityScore"))
    filters.minProductivityScore = parseInt(
      searchParams.get("minProductivityScore")!,
    );
  ...
  if (statsOnly) {
    const stats = extended
      ? getExtendedStats(allScreenshots)
      : getBasicStats(allScreenshots);
    let aggregatedStats;
