...
export default function DashboardStats({ stats, dailyStats }: DashboardStatsProps) {
    ...
    const cards = [
        ...
        },
        {
            ...
            details: (
                <div className="space-y-4">
                    <p className="text-sm text-muted-foreground">Focus score based on screen content & app usage.</p>
                    <div className="flex items-center gap-4">
                        <div className="text-4xl font-bold text-primary">{stats.avgFocus}</div>
        ...
        },
        {
            ...
            details: (
                <div className="space-y-4">
                    <p className="text-sm text-muted-foreground">Productivity score based on categorized activity.</p>
                    <div className="flex items-center gap-4">
                        <div className="text-4xl font-bold text-primary">{stats.avgProductivity}</div>
                        <span className="text-sm text-muted-foreground">/ 100</span>
                    </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {Object.entries(stats.workTypes)
                                    ...
                                    .map(([type, count]) => (
                                        <Badge key={type} variant="secondary" className="flex gap-2">
                                            <span>{type}</span>
                                            <span className="opacity-50 text-[10px]">{count as number}</span>
                                        </Badge>
                                    ))}
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    {Object.entries(stats.apps)
                                        ...
                                        .slice(0, 10)
                                        .map(([app, data]: [string, any]) => (
                                            <div key={app} className="flex justify-between items-center text-sm p-2 rounded hover:bg-muted">
                                                <span>{app}</span>
