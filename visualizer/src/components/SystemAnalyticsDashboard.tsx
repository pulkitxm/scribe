...
export default function SystemAnalyticsDashboard({ stats }: Props) {
  const learningData = Object.entries(stats.learningTopics)
    .map(([name, value]) => ({ name, value }))
    .sort((a, b) => b.value - a.value)
    .slice(0, 10);
  const commData = Object.entries(stats.communicationPlatforms)
    .map(([name, value]) => ({ name, value }))
    .sort((a, b) => b.value - a.value);
  const entData = Object.entries(stats.entertainmentTypes)
    .map(([name, value]) => ({ name, value }))
    .sort((a, b) => b.value - a.value);
  const audioInData = Object.entries(stats.audioInputDevices).map(
    ([name, value]) => ({ name, value }),
  );
  const audioOutData = Object.entries(stats.audioOutputDevices).map(
    ([name, value]) => ({ name, value }),
  );
        ...
        <Card>
          ...
          <CardContent className="h-[250px]">
            <div className="space-y-4 pt-4">
              ...
              {audioInData.length === 0 && (
                <div className="text-center text-muted-foreground">
                  No input devices recorded
                </div>
              )}
            </div>
          </CardContent>
