...
export default function DayOfWeekChart({
  ...
  const sortedData = DAY_ORDER.filter(
    (day) => dataRecord[day] !== undefined,
  ).map((day) => ({
  ...
  const mostActiveDay = sortedData.reduce(
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Activity distribution across weekdays</CardDescription>
      </CardHeader>
          ...
          >
            ...
            <Bar dataKey="count" radius={[4, 4, 0, 0]}>
              {sortedData.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
                  fill={DAY_COLORS[entry.fullDay] || "#888888"}
                />
              ))}
            </Bar>
          </BarChart>
