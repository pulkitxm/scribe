...
export default function AudioVolumeChart({ data }: AudioVolumeChartProps) {
    if (!data || data.length === 0) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle className="text-sm font-medium uppercase text-muted-foreground">Audio Volume</CardTitle>
                </CardHeader>
        ...
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center justify-between">
                    <span className="text-sm font-medium uppercase text-muted-foreground">Audio Volume Over Time</span>
                    <div className="flex gap-4 text-xs text-muted-foreground">
                        <span>Avg: <span className="font-semibold text-foreground">{avgVolume.toFixed(0)}%</span></span>
                        <span>Range: <span className="font-semibold text-foreground">{minVolume.toFixed(0)}-{maxVolume.toFixed(0)}%</span></span>
                    </div>
                </CardTitle>
                <CardDescription>
                    Volume levels throughout the day •
                    {silentPeriods > 0 && <span className="text-muted-foreground ml-1">{silentPeriods} silent hour(s)</span>}
                    {lowVolumePeriods > 0 && <span className="text-muted-foreground ml-2">• {lowVolumePeriods} low volume period(s)</span>}
                    {peakHours.length > 0 && <span className="text-muted-foreground ml-2">• Peak activity ~{avgPeakHour}:00</span>}
                </CardDescription>
            </CardHeader>
                ...
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                        ...
                        <YAxis
                            ...
                            fontSize={11}
                            domain={[0, 100]}
                            label={{ value: 'Volume %', angle: -90, position: 'insideLeft', style: { fontSize: 11 } }}
                        />
                        <Tooltip
