...
export default function DisplayChart({
  ...
  if (hasDarkModeData) {
    const totalDark = darkModeByHour.reduce(
      (sum, d) => sum + d.darkModeCount,
      0,
    );
    const totalLight = darkModeByHour.reduce(
      (sum, d) => sum + d.lightModeCount,
      0,
    );
          ...
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              <span className="text-sm font-medium uppercase text-muted-foreground">
                Dark Mode Usage by Hour
              </span>
              <div className="text-xs text-muted-foreground">
                Dark Mode:{" "}
                <span className="font-semibold text-foreground">
                  {darkModePercent.toFixed(0)}%
                </span>{" "}
                of time
              </div>
            </CardTitle>
        ...
        {hasMonitorData && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                External display usage distribution
              </CardDescription>
            </CardHeader>
            <CardContent className="h-[250px]">
              <ResponsiveContainer width="100%" height="100%">
        ...
        {hasCorrelationData && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Focus score correlation with display setup
              </CardDescription>
            </CardHeader>
            <CardContent className="h-[250px]">
              <div className="space-y-6 pt-8">
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-sm font-bold text-green-500">
                      {externalDisplayCorrelation.withExternal.avgFocus.toFixed(
                        0,
                      )}
                    </span>
                  </div>
                  <div className="w-full bg-muted rounded-full h-3">
                    <div
                      ...
                      style={{
                        width: `${externalDisplayCorrelation.withExternal.avgFocus}%`,
                      }}
                    />
                  </div>
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-sm font-bold text-blue-500">
                      {externalDisplayCorrelation.withoutExternal.avgFocus.toFixed(
                        0,
                      )}
                    </span>
                  </div>
                  <div className="w-full bg-muted rounded-full h-3">
                    <div
                      ...
                      style={{
                        width: `${externalDisplayCorrelation.withoutExternal.avgFocus}%`,
                      }}
                    />
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">
                ...
                <div className="pt-4 border-t border-border">
                  <div className="text-center">
                    {externalDisplayCorrelation.withExternal.avgFocus >
                    externalDisplayCorrelation.withoutExternal.avgFocus ? (
                      <div className="text-sm">
                        ...
                        <span className="text-muted-foreground ml-1">
                          focus boost with external monitor
                        </span>
                      </div>
                    ) : (
