...
export default function NetworkSignalChart({
  data,
  disconnectionCount = 0,
}: NetworkSignalChartProps) {
  ...
  const avgSignal =
    signalDataWithPercent.reduce((a, b) => a + b.signalPercent, 0) /
    signalDataWithPercent.length;
          ...
          <div className="flex gap-4 text-xs text-muted-foreground">
            ...
            {disconnectionCount > 0 && (
              <span className="text-red-500">
                {disconnectionCount} disconnect(s)
              </span>
            )}
          </div>
        </CardTitle>
