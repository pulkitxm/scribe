...
export default function StorageTrendChart({
  ...
  const criticalPeriods = data.filter(
    (d) => (d.free / d.total) * 100 < 10,
  ).length;
        ...
        <CardDescription>
          ...
          {criticalPeriods > 0 && (
            <span className="text-red-500 ml-1">
              ⚠️ {criticalPeriods} critical period(s) (&lt;10% free)
            </span>
          )}
          {criticalPeriods === 0 && minFreePercent < 20 && (
            <span className="text-amber-500 ml-1">
              ⚠️ Low space warning (&lt;20% free)
            </span>
          )}
          {criticalPeriods === 0 && minFreePercent >= 20 && (
            <span className="text-green-500 ml-1">✓ Healthy storage space</span>
          )}
        </CardDescription>
      </CardHeader>
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(
                ...
                const percent = ((value / props.payload.total) * 100).toFixed(
                  1,
                );
