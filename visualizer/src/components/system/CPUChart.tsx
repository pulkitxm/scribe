...
export default function CPUChart({ data }: CPUChartProps) {
    if (!data || data.length === 0) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle className="text-sm font-medium uppercase text-muted-foreground">CPU Usage</CardTitle>
                </CardHeader>
        ...
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center justify-between">
                    <span className="text-sm font-medium uppercase text-muted-foreground">CPU Usage Over Time</span>
                    <div className="flex gap-4 text-xs text-muted-foreground">
                        <span>Avg: <span className="font-semibold text-foreground">{avgCPU.toFixed(1)}%</span></span>
                        <span>Peak: <span className="font-semibold text-foreground">{maxCPU.toFixed(1)}%</span> @{peakHour}:00</span>
                    </div>
                </CardTitle>
                <CardDescription>
            ...
            <CardContent className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={enhancedData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                        ...
                        <YAxis
                            ...
                            fontSize={11}
                            domain={[0, 100]}
                            label={{ value: 'CPU %', angle: -90, position: 'insideLeft', style: { fontSize: 11 } }}
                        />
                        <Tooltip
                            contentStyle={{ backgroundColor: "#1f2937", border: "none", borderRadius: "8px" }}
                            itemStyle={{ color: "#e5e7eb" }}
                            formatter={(value: number | undefined) => value !== undefined ? [`${value.toFixed(1)}%`, 'CPU Usage'] : ['0%', 'CPU Usage']}
