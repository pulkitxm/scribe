...
export default function IdleTimeChart({ distributionData, hourlyIdleData }: IdleTimeChartProps) {
    ...
    if (!hasDistribution && !hasHourlyData) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle className="text-sm font-medium uppercase text-muted-foreground">Activity & Idle Time</CardTitle>
                </CardHeader>
    ...
    const totalActiveTime = hasHourlyData ? hourlyIdleData.reduce((sum, d) => sum + d.activeMinutes, 0) : 0;
        ...
        <div className="space-y-4">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                            <span className="text-sm font-medium uppercase text-muted-foreground">Idle Time Distribution</span>
                            <div className="text-xs text-muted-foreground">
                                Breaks (&gt;5min): <span className="font-semibold text-foreground">{breakCount}</span> ({breakPercent.toFixed(0)}%)
                            </div>
                        </CardTitle>
                        <CardDescription>
                            ...
                            <BarChart data={distributionData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                ...
                                <Bar dataKey="count" name="Idle Periods" radius={[4, 4, 0, 0]}>
                                    {distributionData.map((entry, index) => (
                                        <Cell key={`cell-${index}`} fill={getColor(entry.range, entry.isBreak)} />
                                    ))}
                                </Bar>
                            </BarChart>
                        </ResponsiveContainer>
            ...
            {hasHourlyData && (
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                            <span className="text-sm font-medium uppercase text-muted-foreground">Active vs Passive Time by Hour</span>
                            <div className="text-xs text-muted-foreground">
                                Active Work: <span className="font-semibold text-foreground">{activePercent.toFixed(0)}%</span>
                            </div>
                        </CardTitle>
                        <CardDescription>
                            Breakdown of active work time vs passive viewing throughout the day
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="h-[300px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={hourlyIdleData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                                <XAxis
                                    dataKey="hour"
                                    stroke="#888888"
                                ...
                                <Tooltip
                                    contentStyle={{ backgroundColor: "#1f2937", border: "none", borderRadius: "8px" }}
                                    itemStyle={{ color: "#e5e7eb" }}
                                    formatter={(value: number | undefined, name: string | undefined) => value !== undefined ? [`${value.toFixed(1)} min`, name || ''] : ['0 min', name || '']}
                                    labelFormatter={(label) => `Hour: ${label}:00`}
                                />
                                <Legend />
                                <Bar dataKey="activeMinutes" name="Active Work" stackId="a" fill="#10b981" radius={[0, 0, 0, 0]} />
                                <Bar dataKey="passiveMinutes" name="Passive/Idle" stackId="a" fill="#f59e0b" radius={[4, 4, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </CardContent>
                </Card>
