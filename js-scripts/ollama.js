...
Pre-extracted text (priority 1 = largest/biggest text on screen):
{EXTRACTED_TEXT}
...
function callOllama(imageBase64, opts = {}) {
  const { isRetry = false, extractedText = '' } = opts;
...
Use a single category label only.`;
    ...
    const promptText = (PROMPT.replace('{EXTRACTED_TEXT}', extractedText || '(none)'));
    const finalPrompt = isRetry ? `${retryHint}\n\n${promptText}` : promptText;
